...
Android APK wrapper for the BTCPC PWA (`website/app.html`) using Capacitor.
...
The APK is copied to `../btcpc/website/btcpc.apk` for download from the site.
...
```bash
keytool -genkey -v -keystore btcpc-release.jks -keyalg RSA -keysize 2048 -validity 10000 -alias btcpc
```

Then build:

```bash
BTCPC_KEYSTORE=./btcpc-release.jks npm run build:release
```
