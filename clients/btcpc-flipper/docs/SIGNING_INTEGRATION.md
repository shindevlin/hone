...
> a Flipper Zero becomes a `SensorReading` ledger entry the BTCPC chain will
...
> `protocol/btcpc_protocol.h` and identity/BLE scenes). This doc is the contract
...
- **Flipper side (exists today):** the Flipper generates an ed25519 identity
  keypair on first run (hardware TRNG), and `btcpc_protocol.h` signs the **BLE
  frame payload bytes** — a packed `BtcpcSubGhzObs` / `BtcpcNfcScan` / etc.
...
1. **Flipper → phone (device key, existing mechanism):** the Flipper captures a
   reading, packs it into the existing `BtcpcSubGhzObs`/`BtcpcNfcScan`/etc.
   struct, and signs the BLE frame payload with its **device key**, exactly as
   `btcpc_protocol.h` already specifies. No firmware signing change needed.
...
3. **Phone → chain (owner posting key):** the phone translates the verified
   ...
   after PR #7. The phone (`btcpc-android`) is the component that actually holds
...
**Chain (`btcpc-node`):** DONE in PR #7 (`9e393c65`). No further change for
...
- Drive each radio/reader (sub-Ghz, NFC/ISO14443, RFID/125kHz, iButton, IR) to
  populate the *already-defined* payload structs in `btcpc_protocol.h`.
...
**Phone (`btcpc-android`) — the integration work:**
