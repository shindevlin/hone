/*
 * btcpc_scene_ble.c — BLE pairing status scene
 *
 * Shows the current BLE connection state and instructs the user
 * how to pair. The actual BLE NUS peripheral is managed in btcpc.c;
 * this scene is display-only.
 *
 * TODO (Phase 9 wiring):
 *   - Start BLE NUS peripheral via furi_hal_bt_start_advertising()
 *     or profiles/serial_profile.h (matches btcpc_ble_relay pattern)
 *   - Register RX callback to receive BTCPC_MSG_ENTRY_HASH /
 *     BTCPC_MSG_CLOCK_SYNC / BTCPC_MSG_GPS frames from the phone app
 *   - On each incoming Sub-GHz / RFID / NFC / iButton event, call
 *     btcpc_build_*() and send over BLE TX characteristic
 *   - Auto-reconnect: store paired device address in
 *     /ext/apps_data/btcpc/paired.addr (6 bytes) and reconnect on boot
 *
...
bool btcpc_scene_ble_on_event(void* context, SceneManagerEvent event) {
    ...
    UNUSED(event);
    /*
     * TODO: handle BtcpcEventBlePaired / BtcpcEventBleDisconnected custom
