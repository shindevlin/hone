...
One-click mining for [Bitcoin Proof of Compute](https://btcpc.net). Native desktop app built with Tauri (Rust + HTML).

## What it does

- Auto-detects your hardware (CPU, GPU, RAM)
- Recommends miner or clock node mode
- Installs BTCPC node code with one click
...
- **`src-tauri/`** — Rust backend with Tauri commands
  ...
  - `installer.rs` — clones/updates BTCPC repo, runs npm install
...
| `install_btcpc` | Clone repo + npm install |
