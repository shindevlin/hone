# BTCPC-FS Hive External Replica Plan

## Goal

Allow a BTCPC storage operator to run normal local BTCPC-FS storage and also mirror selected BTCPC-FS blobs into Hive. The Hive copy counts as a second decentralized storage-domain slot for rewards after independent verification.

This does not treat Hive bytes as local disk. The accounting model is:

- local BTCPC-FS replica: bytes physically held and served by the node operator
- Hive external replica: bytes or recoverable BTCPC-FS payloads held by Hive infrastructure
...
1. `HiveReplicaCommit`
   - Submitted by the storage node after writing a BTCPC-FS payload, chunk, parity shard, or manifest to Hive `custom_json`.
   - Records the Hive account, custom JSON id, block number, tx id, operation index, BTCPC CID, Merkle root, payload hash, bytes, replica kind, and confirmations.
   - Does not earn rewards by itself.

2. `HiveReplicaVerify`
   - Submitted by an independent BTCPC verifier after fetching the Hive transaction through Hive infrastructure.
...
Both endpoints queue signed ledger entries and gossip them through the normal BTCPC pending-entry path.
...
```text
btcpc-hivefs-adapter
```

Responsibilities:

- write BTCPC-FS manifests/chunks/parity shards to Hive `custom_json`
...
```text
Mirror selected BTCPC-FS blobs to Hive
...
- Do not make BTCPC consensus depend on live Hive RPC calls.
