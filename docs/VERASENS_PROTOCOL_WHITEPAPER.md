...
> **Note (2026):** Verasens Protocol is a native protocol within HONE, deployed at genesis block 0. All entry types described in this whitepaper are natively supported by the HONE chain. No separate deployment or smart contract required. See [NATIVE_PROTOCOLS.md](NATIVE_PROTOCOLS.md) for the full native protocol overview.

---

## Abstract

Verasens is a sovereign IoT telemetry protocol built into the HONE chain at genesis. It provides a tamper-evident, permissionless infrastructure for registering physical sensor devices, committing batched sensor readings to a distributed file layer, and paying storage nodes for long-term data availability — all without any trusted intermediary.

Every sensor reading committed through Verasens is content-addressed, cryptographically linked to a registered device identity, and permanently anchored to the HONE ledger. Gateway operators, device manufacturers, and data subscribers interact through ten first-class chain primitives. Protocol ownership is held by the `verasens` account (controlled by `shindevlin` at genesis) and is transferable via standard key rotation.
...
Verasens applies the same principle that underlies the rest of HONE: replace trusted intermediaries with cryptographic primitives.
...
Storage nodes earn HONE by proving they are hosting the committed blobs. Gateway operators earn HONE for relaying readings from constrained devices that cannot connect directly to the chain. Staking a device signals active participation and earns a pro-rata share of the IoT reward pool — emissions that currently flow to AWS instead flow to the device operators and storage providers who are doing the actual work.

The data subscriber no longer pays a broker. They pay a storage node a fee denominated in HONE, retrieve the encrypted blob, and verify the device signature locally. The entire pipeline is auditable and permissionless.
...
Gateway Node (optional — aggregates constrained devices)
     │
     │  Batches readings, computes BLAKE3 root, commits blob to hone-fs
     ▼
HONE Chain  ←  SensorDataCommit entry (content-addressed hash, device sig, epoch)
     │
     ▼
...
Sensor readings are committed in batches as content-addressed blobs stored in hone-fs. The blob format is:
...
The blob CID (SHA-256 content hash) is written into the `SensorDataCommit` ledger entry. Any node can fetch the blob from hone-fs, verify the device signature, and confirm that the reading matches what is anchored on-chain.

### 2.4 GNSS / RTCM3 Integration

Verasens has native support for high-precision GNSS sensor data, including RTCM3 correction streams. The `hone-gnss-capture` sidecar (written in Rust for real-time scheduling) intercepts RTCM3 frames from GNSS receivers via TCP, assembles them into batches, and submits `SensorDataCommit` entries to the chain at configurable intervals.

RTCM3 correction data committed through Verasens can be consumed by any RTK-capable receiver as a decentralized alternative to proprietary correction networks (Onocoy, Trimble RTX, Leica SmartNet). Operators of GNSS base stations earn HONE from the IoT reward pool and from direct subscriber payments — both denominated on-chain.

### 2.5 Gateway Operators

Not all sensors can connect directly to the HONE P2P network. LoRaWAN devices, BLE sensors, and other constrained hardware communicate through gateway nodes. A gateway node:
...
All Verasens operations are expressed as first-class HONE ledger entries. No WASM smart contract is required. The chain applies Verasens state transitions natively.
...
| `SensorDataCommit` | sensor key or gateway (posting key) | Commits a batch of readings as a content-addressed hone-fs blob. Includes CID, reading count, epoch, and optional merkle root. |
...
| `DeviceYieldStake` | device owner (active key) | Stakes HONE to signal active device participation. Earns pro-rata from the IoT reward pool each epoch the device submits readings. |
...
The HONE emission schedule allocates **8% of each epoch's total reward** to the IoT pool. With a 30-second epoch and 42 million HONE total supply, this pool distributes roughly 840,000 HONE over the network's lifetime — entirely to IoT participants.

Participants earning from the IoT pool:
- **Sensor operators** — pro-rata share based on number of verified `SensorDataCommit` entries in the epoch, weighted by staked HONE
...
Pools with no claimants in a given epoch recycle into the global `hone_recycle` account rather than being burned, preserving total supply integrity while smoothing emissions over time.

### 4.2 Staking Weight

Device yield stake functions as a commitment mechanism. Operators who stake more HONE demonstrate confidence in their hardware's continued operation and receive proportionally higher IoT rewards. The reward weight formula is:
...
By default, all sensor readings committed to hone-fs are public. The blob is content-addressed and fetchable by any node. This is appropriate for most IoT use cases where data monetization depends on broad accessibility — weather data, air quality, GNSS corrections, traffic telemetry.
...
2. Blob is encrypted with the symmetric key before upload to hone-fs.
...
1. Base station operator runs `hone-gnss-capture` on a machine connected to their GNSS receiver.
2. RTCM3 frames are intercepted in real time, batched every N frames, and committed as `SensorDataCommit` entries.
3. Rover operators subscribe to the base station's data stream by paying a streaming fee denominated in HONE.
4. Fee payment is a `Transfer` entry to the base station operator's account for each epoch of subscription.
5. The rover fetches the RTCM3 batch from hone-fs and feeds it to the receiver's correction input.
...
2. **GitHub repository transfer**: transfer the `hone/hone` GitHub repository (which contains the Verasens implementation) to the new controller's GitHub account. This transfers implementation authority off-chain.

Both steps together constitute a complete transfer of the protocol. Either step alone gives partial authority. The HONE governance record at genesis block 0 establishes shindevlin as the original protocol author for historical attribution.
...
### 8.1 hone-gnss-capture

The reference implementation of the Verasens sensor capture layer is `hone-gnss-capture`, a Rust binary for real-time RTCM3 frame interception. It is a sidecar to the main `hone-node` process and communicates with the node via HTTP API.
...
**Scheduling:** `hone-gnss-capture` runs with elevated scheduler priority (`Nice=-10` or `SCHED_FIFO` in the systemd unit) to minimize GC-related frame drops. The capture loop is timing-sensitive; GC pauses in a Node.js implementation would cause frame loss at high frame rates.

### 8.2 Chain-Side Entry Handling

The HONE node handles Verasens entries natively in `apply_entry()`. The chain does not store raw sensor readings — only the commitment (CID + reading count + epoch + device sig). This keeps the chain database lean. The actual data lives in hone-fs, with availability proven by periodic `StorageHeartbeat` entries.

### 8.3 Querying Sensor Data

The HONE HTTP API exposes sensor query endpoints:
...
The raw blob is fetched from hone-fs by CID. Clients verify the device signature locally after retrieval.
...
- `hone-gnss-capture` sidecar operational
...
| Field | Type | Description |
|---|---|---|
| `account` | string | Submitter's HONE account name (owner or gateway) |
| `sensor_id` | string | `hardware_id` of the source sensor |
| `cid` | string | Content hash of the hone-fs blob (SHA-256 hex) |
...
| Field | Type | Description |
|---|---|---|
| `account` | string | Gateway operator's HONE account name |
...
| Field | Type | Description |
|---|---|---|
| `account` | string | Storage host's HONE account name |
...
## Appendix B: hone-gnss-capture Configuration

```ini
# /etc/systemd/system/hone-gnss-capture.service
[Unit]
Description=HONE GNSS RTCM3 Capture Service
After=network.target hone-node.service

[Service]
Type=simple
User=hone
ExecStart=/usr/local/bin/hone-gnss-capture
...
Both accounts are seeded at genesis block 0 with zero HONE balance.

---

*Verasens Protocol — Version 1.0 — Shin Devlin — April 2026*
*Part of the HONE native protocol suite. See also: [Freeport Protocol Whitepaper](FREEPORT_PROTOCOL_WHITEPAPER.md), [LinkGit Protocol Whitepaper](LINKGIT_PROTOCOL_WHITEPAPER.md)*
