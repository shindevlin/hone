---
title: HONE Inference Migration — running notes & decision log
...
> findings, and gotchas for moving HONE inference off Ollama to embedded candle.
...
- **We ALREADY run candle for GGUF.** `rust/hone-android/Cargo.toml` uses
  `candle-core`/`candle-transformers`/`candle-nn`, and
  `rust/hone-android/src/llm.rs` is a COMPLETE working candle GGUF inference
...
## 3. The gift: hone-android/src/llm.rs already does this
The hard part is written. `rust/hone-android/src/llm.rs` (verified 2026-07-04):
...
**The node migration = port this into hone-node behind the unified inference
...
2. **Add embedded candle backend** behind cargo feature `inference-embedded`,
   porting hone-android/src/llm.rs. inference.rs picks Embedded when built with
...
- **rustc pin:** the node is pinned to rustc 1.90.0 (rust-toolchain.toml) due to
  ...
  on 1.90 before committing to it. (hone-android uses candle 0.10 — check what
...
- `rust/hone-android/src/llm.rs` — **candle** GGUF (Qwen2.5-0.5B). This is the
  one to standardize on and port to the node.
- `android/rust/hone-miner/src/miner.rs` — **tract-onnx** (loads .onnx). Older
  path used by the Java app's libhone_miner.so.
Standardizing the node on candle means the whole ecosystem converges on candle
GGUF; the tract-onnx path in hone-miner should eventually be retired/migrated to
...
- **2026-07-04:** Decision made (candle-embedded). Analysis docs written
  ...
  Found hone-android/src/llm.rs as the proven port source. bullship inference
...
- **2026-07-04 (build started):** Shin said "just do candle, no wasted cycles" —
  ...
  - Added candle-core/transformers/nn 0.10 + tokenizers 0.20 + hf-hub 0.3 to
    hone-node/Cargo.toml (same versions as hone-android, which builds), all
