...
| `SensorDataCommit` | sensor key or gateway | Commits a batch of readings as a content-addressed blob; links to HONE-FS |
| `DeviceKeyRegister` | device owner | Registers an Ed25519 key for a general IoT device (non-sensor) |
| `DeviceYieldStake` | device owner | Stakes HONE to signal active participation; earns pro-rata from IoT reward pool |
...
| `StorageHeartbeat` | storage host | Proves continued availability of sensor data blobs committed to HONE-FS |
...
*See also: [[HONE_WHITEPAPER]], [[PLAN_v2.10.1_to_v2.14]] §v2.15 (HONE-nano + LoRa sensor mesh)*
