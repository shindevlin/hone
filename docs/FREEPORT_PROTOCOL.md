...
| **seller** | `OrderFulfill`, `EscrowRelease` — auto-fulfills digital orders and releases escrow |
| **buyer** | Receives encrypted digital product payload; decrypts with their private key |
...
| `OrderFulfill` | seller | Marks order fulfilled; for digital goods, attaches encrypted payload |
| `OrderCancel` | buyer or seller | Cancels an open order before fulfillment |
| `OrderDispute` | buyer | Escalates an order to dispute state; pauses escrow release |
| `EscrowRelease` | seller (auto) or arbitrator | Releases escrowed funds after fulfillment or dispute resolution |
...
1. Buyer submits `OrderPlace` with their Ed25519 public key included in the entry payload.
2. Payment is locked in `freeport-escrow` until the order is resolved.
3. Seller's node detects the order, encrypts the product payload using the buyer's public key (X25519 ECDH + AES-256-GCM).
4. Seller signs and submits `OrderFulfill` with the ciphertext attached.
5. Buyer fetches the entry from any chain node and decrypts locally with their private key.
