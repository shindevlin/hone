...
- New runtime ledger entry variants in `btcpc-types`.
...
## Verification Run
- Passed:
  - `cargo test -p btcpc-types --manifest-path rust/btcpc-node/Cargo.toml`
  - `cargo check -p btcpc-node --manifest-path rust/btcpc-node/Cargo.toml`
