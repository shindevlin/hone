...
### VEC-8 — `/v1/chat/completions` Bearer token equals public account name
**File:** `src/api.rs` — `post_v1_chat_completions`  
**Risk:** Medium  
The auth check `Authorization: Bearer <token>` treats the token as an account name and verifies only that the account exists with sufficient balance. Account names are public on-chain. Any party who knows your account name can debit your balance by making inference requests on your behalf.  
**Daemon guard:** N/A — this is a direct API surface  
**Chain guard:** N/A — auth is enforced in the API layer, not the state machine  
**Fix requires:** Per-account API tokens registered on-chain via a new `AccountApiKeySet` entry. Token is a random 256-bit secret; chain stores `api_key:{token}` → account. Bearer token is the random secret, not the public account name.  
**Watch when:** Before mainnet. Before per-account balances grow beyond faucet drips. Particularly critical once external projects (PM-100 style integrations) store API keys in `.btcpc/wallet.env` — if account names are guessable or enumerable, the entire balance of any API-using account is drainable.
