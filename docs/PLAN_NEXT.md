...
- Create `honemesh.net/hone-image-version.txt` — a tiny file with the current image sha256
- Add `bin/hone-auto-update` — a script that runs inside the container every hour:
...
- Wire into `bin/hone-all` as a background loop (not a separate role — runs alongside all roles)
- **Files**: new `bin/hone-auto-update`, edit `bin/hone-all`
...
- Build a `hone-clock:latest` mini Docker image (~50 MB) for clock-only users
- Move `ollama` service in docker-compose.yml to `profiles: [miner]`
- **Files**: `website/hone-start.bat`, `.ps1`, `install.sh`, `website/docker-compose.yml`, `Dockerfile.clock` (new)
...
- Re-save tarball to `website/hone-image.tar.gz`
- Update `hone-image-version.txt`
- **Files**: `website/hone-image.tar.gz` (gitignored), new `website/hone-image-version.txt`
...
- New `src/routes/bridgeRoutes.js`:
  - POST /api/bridge/wrap — wrap HONE to wHONE
  - POST /api/bridge/unwrap — unwrap wHONE to HONE
...
### 2.4 Sensor data → HONE-FS persistence
...
### 3.2 Solidity contracts for wHONE
The actual ERC20 contracts for the bridge.

- `contracts/wHONE.sol` — ERC20 with 4.2M supply, no mint, no burn, just transfers
