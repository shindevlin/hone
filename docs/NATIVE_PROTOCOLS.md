...
BTCPC ships with three protocol businesses baked in from genesis: Freeport, Verasens, and LinkGit. They are not add-ons, not third-party integrations, and not future roadmap items. They exist from block 0.

**Why native matters:**

- No bridge required — every transaction settles directly on the BTCPC chain
- No separate token — fees, escrow, and staking all flow through BTCPC
...
Each protocol is independently valuable. Each is also independently licensable. Together they give BTCPC three separate, non-correlated demand drivers, any one of which would justify the chain on its own.
...
- Every purchase locks BTCPC in escrow for the duration of the transaction — driving sustained token lock-up proportional to commerce volume
...
**What it is:** Verasens is a decentralized IoT sensor and device data protocol. It turns the BTCPC chain into a verifiable ledger for real-world sensor data from any registered device.
...
- Device operators stake BTCPC via `DeviceYieldStake` to earn emissions for data contributions
...
- Sensor staking requirements — devices must hold BTCPC to participate, reducing circulating supply in proportion to the size of the sensor network
...
**What it is:** LinkGit is a decentralized git protocol. It stores repositories as content-addressed objects on BTCPC-FS and records all ref updates on-chain.
...
- Per-object storage fees paid in BTCPC for every blob, tree, and commit stored on BTCPC-FS
...
The three protocols create three separate, independent demand drivers for BTCPC:

| Protocol | Demand Mechanism | Token Lock |
|----------|-----------------|------------|
| Freeport | Every purchase locks BTCPC in escrow | Duration of transaction |
| Verasens | Sensor staking requires BTCPC to participate | Duration of device operation |
| LinkGit | Storage fees paid in BTCPC per object | Ongoing per-repo |
...
Combined, they make BTCPC a utility token in the strict sense: holding BTCPC is required to do things people want to do.

---

## Independence

Each protocol is native to BTCPC by choice, not by technical constraint.

Freeport's escrow mechanics, Verasens's device staking model, and LinkGit's content-addressed storage could each be deployed on any chain with sufficient storage and scripting capability. They run on BTCPC because BTCPC was designed to support exactly this kind of work-denominated, fee-generating protocol from the start.

This also means each protocol has standalone licensing value. An enterprise that wants on-chain sensor data integrity but does not want to run a full BTCPC node could license Verasens as a module. A company that wants censorship-resistant code storage could license LinkGit. A marketplace operator could license Freeport's escrow and delivery infrastructure.

Being native to BTCPC from genesis means these protocols are battle-tested on a live chain with real blocks, real fees, and real token mechanics — not whitepaper designs.
