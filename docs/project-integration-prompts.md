# HONE Project Integration Prompts

Each prompt below is designed to be given to a Claude/Codex session working on that specific project. The goal: make each project actually USE the HONE inference results and submit meaningful new inference jobs.

Every project has a `.hone-inference/` directory with completed AI inference results (JSON files). The integration should:
1. Read completed results and use them in the project
2. Submit new inference jobs that serve the project's actual purpose
3. Pay for inference from the project's HONE balance
...
HONE INTEGRATION:
The .hone-inference/ directory contains AI-generated fake crypto headlines from the HONE network. These are EXACTLY what this game needs.

TASKS:
1. Read all JSON files from .hone-inference/ and extract the "result" field
2. Parse each result as a fake headline and store in the game's headline database
3. Create a script that submits NEW headline generation jobs to HONE:
...
5. The game should display both real headlines (from news APIs) and HONE-generated fakes

The project's HONE API key is in .envhone (HONE_PROJECT_KEY).
```
...
HONE INTEGRATION:
The .hone-inference/ directory contains AI-generated creative writing (romance stories, character encounters, descriptive scenes). These can be published as community stories.

TASKS:
1. Read completed inference results and format them as publishable stories
2. Add metadata: title (extract from first line), author ("HONE AI"), genre, word count
3. Store in the platform's story database/content system
4. Create a story generation pipeline that submits to HONE:
...
6. Add a "AI Stories" section to the platform showing HONE-generated content

The project's HONE API key is in .envhone.
```
...
HONE INTEGRATION:
The .hone-inference/ directory contains AI-generated sports commentary and analysis.
...
6. Display HONE-powered analysis alongside aggregated real news
...
HONE INTEGRATION:
The .hone-inference/ directory contains Linux admin tutorials and howto guides.
...
3. If no cached answer, submit a new inference job to HONE in real-time:
...
6. The bot becomes a HONE-powered sysadmin assistant
...
HONE INTEGRATION:
The .hone-inference/ directory contains character monologues (Victorian detective, self-aware AI, last librarian).
...
6. Each generation is a HONE inference job — users see the cost and transaction
...
HONE INTEGRATION:
The .hone-inference/ directory contains PII detection results (SSN, addresses, medical data identified).

TASKS:
1. Read inference results and extract identified PII patterns
2. Build a pattern library from successful detections
3. Integrate HONE as a real-time PII detection backend:
   - User submits document text → HONE inference identifies PII → redact
   - Much more accurate than regex-only detection
4. Create batch processing: scan a directory of documents via HONE inference
...
HONE INTEGRATION:
The .hone-inference/ directory contains competitive analysis, SWOT analysis, and business metrics guidance.
...
3. Create an on-demand analysis pipeline:
   - User inputs a business idea or industry → HONE generates analysis
...
5. Each analysis shows its HONE cost and inference details
...
HONE INTEGRATION:
The .hone-inference/ directory contains sports odds analysis and betting commentary.
...
4. Bot command: /analyze [matchup] → submits HONE inference → returns analysis
...
HONE INTEGRATION:
The .hone-inference/ directory contains supportive counseling-style responses (from generic prompts).
...
3. Create an API endpoint: POST /api/legal-query → HONE inference → formatted legal guidance
4. Index results by jurisdiction, procedure type, and document type
5. Each query shows the HONE transaction for transparency
...
HONE INTEGRATION:
The .hone-inference/ directory contains motivational crypto content and builder philosophy.
...
3. Generate random events powered by HONE inference
4. Dynamic content: each player session gets unique AI-generated text
5. The game itself runs on HONE compute — meta!
...
HONE INTEGRATION:
The .hone-inference/ directory contains content about digital ownership and creator licensing.
...
3. Create AI-powered features:
   ...
   - Hashtag suggestions
   - Reply drafts
4. Each AI feature is a HONE inference call — users see the compute cost
...
HONE INTEGRATION:
The .hone-inference/ directory contains waitlist emails and marketing copy.
...
3. Integrate into the waitlist platform: when a new project is created, auto-generate email templates via HONE
4. A/B test AI-generated vs human-written copy (track open/click rates)
5. Each generated email shows its HONE cost
...
2. Read `.hone-inference/*.json` to see what inference results look like
3. Implement the integration described in the prompt
4. Test by reading existing results AND submitting a new inference job
5. The HONE API is at `http://localhost:3000/v1/inference/submit` (Bearer token from `.envhone`)
