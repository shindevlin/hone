...
HONE should never have permanently lost tokens.
...
4. **Decay destination**: decayed tokens go to `hone_recycle` — a protocol address that feeds back into the block reward pool
...
```
Year 0:  Alice has 1000 HONE, goes inactive
Year 5:  Grace period ends. Balance still 1000 HONE.
Year 6:  10% decay: 900 HONE remains, 100 recycled
Year 7:  10% decay: 810 HONE remains, 90 recycled
Year 8:  10% decay: 729 HONE remains, 81 recycled
Year 10: 590 HONE remains (59% of original)
Year 15: 349 HONE remains (35%)
Year 20: 206 HONE remains (21%)
Year 30: 72 HONE remains (7.2%)

If Alice comes back at year 10: she has 590 HONE.
She taps /heartbeat → clock resets. 590 HONE is hers for another 5 years.
```
...
2. For accounts with `lastActivity < now - 5 years`:
   ...
   - Record ledger entry: `DORMANCY_DECAY from: account, to: hone_recycle`
   - Update SMT
3. `hone_recycle` balance is added to the next epoch's block reward
...
- CLI: `node bin/hone-cli wallet heartbeat`
...
- Burn address (`hone_burn`) tokens also decay → recycle (even burns aren't permanent)
- Genesis accounts (shindevlin, reserved names) follow the same rules — no exceptions
- Notification at 4.5 years: Telegram, email, explorer — "tap to keep your HONE active"

### Automatic Heartbeat — Cross-Chain Proof of Life

The most elegant approach: if you're active on ANY linked blockchain, your HONE dormancy clock resets automatically. You never need to think about HONE specifically.

**How it works:**

Every HONE account has deterministic cross-chain addresses (EVM, Solana, Bitcoin, TON, Hive) derived from the same mnemonic. The HONE network already monitors these addresses for cross-chain mining claims. The same watcher can detect activity:

```
User makes an ETH transfer on Ethereum
  → HONE cross-chain watcher sees activity on the linked EVM address
  → Automatically writes a HEARTBEAT entry on the HONE ledger
  → Dormancy clock resets for 5 years
  → User never touched HONE directly
```
...
If your linked ETH wallet is active but you haven't signed with your HONE key, your tokens are **frozen** (no decay, but can't spend). This protects against the scenario where someone lost their HONE mnemonic but their ETH wallet is still active — the tokens are preserved but locked.

To spend frozen tokens: sign any HONE transaction (heartbeat, transfer, stake) with the native HONE key.
...
| Years 5-10 | **Frozen** — safe, no decay, can't spend until HONE native sign | **Decaying** — 10%/year of remaining |
| Years 10+ | **Decaying** — cross-chain alone isn't enough anymore, 10%/year | Still decaying |

At ANY point: one native HONE signature (heartbeat, transfer, anything) → unfrozen, clock fully resets, keep whatever remains.

```
Year 0:   1000 HONE, active
Year 5:   No HONE activity, ETH wallet active → frozen (1000, locked)
Year 8:   Still frozen, still 1000 HONE — cross-chain protecting it
Year 10:  Cross-chain grace expires → decay starts (1000 → 900)
Year 11:  900 → 810
Year 15:  590 HONE remaining
Year 12:  User signs with HONE key → unfrozen, 810 HONE, clock resets for 5 years
```

### Other Heartbeat Methods

**1. On-chain recurring contract (set-and-forget)**
Register a `HEARTBEAT_CONTRACT` once. The protocol auto-heartbeats your account every 4 years during epoch finalization. Costs 0.01 HONE one-time fee. Cancel anytime.
...
| Recurring contract | Zero after setup | 0.01 HONE one-time | Set-and-forget |
