 ...
 * Miners accumulate cross-chain credits on the BTCPC chain (0.1 BTCPC per chain per
 * BTCPC mined), then claim in bulk at any time using a signed proof from the BTCPC oracle.
 *
 * Fee model:
 ...
 * the BTCPC chain when recordCrossChainClaim is called. Used nonces are stored
 * permanently — one nonce, one mint.
 */
contract wBTCPC is ERC20, Ownable, Pausable {
    ...
    /// @notice Nonces issued by the BTCPC chain — each usable exactly once
    mapping(bytes32 => bool) public usedNonces;

    event Claimed(address indexed claimer, string btcpcAccount, uint256 amount, bytes32 nonce, uint256 feePaid);
    ...
    /**
     * @notice Claim accumulated wBTCPC rewards.
     *
     * @param btcpcAccount  The BTCPC account name (e.g. "shindevlin"), packed as bytes32
     * @param amount        Raw token units (10 decimal fixed-point)
     * @param nonce         One-time nonce issued by the BTCPC chain for this claim
     * @param signature     ECDSA (EIP-191) signature from signingAuthority over
     *                      keccak256(chainId, msg.sender, btcpcAccount, amount, nonce)
     *
     ...
     */
    function claim(
        bytes32 btcpcAccount,
        uint256 amount,
        bytes32 nonce,
        bytes calldata signature
    ) external payable whenNotPaused {
        require(!usedNonces[nonce],  "wBTCPC: nonce already used");
        ...
        require(_recover(ethHash, signature) == signingAuthority, "wBTCPC: invalid signature");
