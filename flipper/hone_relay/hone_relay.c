/**
 * hone_relay.c — HONE Relay Flipper Zero app
 *
 * Announces the Flipper's BLE MAC over USB CDC so the PC pairing tool
 * can push it to the HONE Android app.  If a posting key is stored in
 * /int/.hone/posting.key, the app computes HMAC-SHA256(mac, key) and
 * includes the signature — the key itself never leaves the Flipper.
 *
 * Announce format (one line, newline-terminated):
 *   With key:    HONE_MAC:<mac> HONE_SIG:<64-char-hex-hmac>\n
 *   Without key: HONE_MAC:<mac>\n
 *
 ...
 *
 * Key file: /int/.hone/posting.key
 *   - Flipper internal flash, NOT accessible via USB mass storage
 *   - NOT visible in the Flipper file browser
 *   - Written once by the honeflipper pairing tool: scripts/set-flipper-key.js
 *
...
#define TAG            "HONERelay"
#define PAGE_COUNT     5
#define KEY_PATH       "/int/.hone/posting.key"
...
static void draw_callback(Canvas* c, void* ctx) {
    ...
    switch(app->page) {
    case 0:
        draw_header(c, "HONE Relay");
        canvas_set_font(c, FontSecondary);
        canvas_draw_str(c, 2, 26, "Earn HONE by relaying");
    ...
    case 2:
        ...
        canvas_draw_str(c, 2, 60, "this in the HONE app.");
        draw_nav(c, true, true);
        break;
    case 3:
        ...
        canvas_draw_str(c, 2, 35, "  Open HONE app");
    ...
    case 4:
        draw_header(c, "Ready");
        canvas_set_font(c, FontSecondary);
        canvas_draw_str(c, 2, 25, "Waiting for HONE app...");
