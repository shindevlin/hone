/**
 * hone_ble_relay.c — HONE BLE Relay Flipper app
 *
 * HONE-branded replacement for the stock UART BLE Bridge.
 * Starts BLE NUS (Nordic UART Service) peripheral so the HONE Android
 ...
 *   /int/.hone/posting.key  — 32-byte hex HMAC key (optional; set by pair-flipper.js)
 *   /int/.hone/account.txt  — HONE account name, e.g. "josh" (optional; default = "user")
 *
 * Screen:
 *   ┌──────────────────────────────┐
 *   │ HONE BLE Relay              │
 ...
 *   └──────────────────────────────┘
 *
 * Build:  cd flipper/hone_ble_relay && ufbt build
 */
...
#define TAG               "HONEBLERelay"
#define KEY_PATH          "/int/.hone/posting.key"
#define ACCOUNT_PATH      "/int/.hone/account.txt"
...
static void draw_cb(Canvas* c, void* ctx) {
    ...
    canvas_draw_str(c, 2, 11, "HONE BLE Relay");
...
int32_t hone_ble_relay_app(void* p) {
    UNUSED(p);
    ...
    /* USB CDC — take over serial port so the hone-flipper-listener on the
