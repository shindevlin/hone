...
#include "btcpc_sensor.h"
#include "qrcodegen.h"

#define BTCPC_VIEW_MENU 0
#define BTCPC_VIEW_RESULT 1
#define BTCPC_VIEW_PAIR 2
#define BTCPC_VIEW_DIAGNOSTICS 3

#define BTCPC_MENU_BATTERY 0
#define BTCPC_MENU_CPU_TEMP 1
#define BTCPC_MENU_GPIO 2
#define BTCPC_MENU_NFC 3
#define BTCPC_MENU_BLE 4
#define BTCPC_MENU_SUBGHZ 5
#define BTCPC_MENU_CYCLE 6
#define BTCPC_MENU_PAIR 7
#define BTCPC_MENU_UNPAIR 8
#define BTCPC_MENU_DIAGNOSTICS 9
#define BTCPC_MENU_DIAGNOSTICS_SEND 10

static const char* BTCPC_SUBGHZ_DEVICE = "cc1101_int";
static const uint32_t BTCPC_SUBGHZ_FREQ = 433920000UL;

static const uint32_t btcpc_cycle_order[] = {
    BTCPC_MENU_BATTERY,
    BTCPC_MENU_CPU_TEMP,
    BTCPC_MENU_GPIO,
    BTCPC_MENU_NFC,
...
static void refresh_pair_id(BtcpcSensorApp* app) {
    if(!app) return;
    uint32_t next = furi_hal_random_get();
    if(next == 0) next = 0xB7C0C001u;
    app->pair_id = next;
    app->beacon_seq = 1;
    snprintf(app->pair_token, sizeof(app->pair_token), "BTCPC:PAIR:%08lX", (unsigned long)app->pair_id);
}
...
static bool load_or_create_posting_key(BtcpcSensorApp* app) {
    if(!app || !app->storage) return false;

    File* file = storage_file_alloc(app->storage);
    if(!file) return false;

    const char* key_path = APP_DATA_PATH("btcpc_sensor.key");
    bool ok = false;
    uint8_t key[32] = {0};
...
static void init_device_identity(BtcpcSensorApp* app) {
    if(!app) return;
...
static void update_pair_view_model(BtcpcSensorApp* app, const char* title) {
    if(!app || !app->pair_view) return;
    BtcpcPairViewModel* model = view_get_model(app->pair_view);
    if(!model) return;
...
static void draw_qr(Canvas* canvas, const BtcpcPairViewModel* model) {
...
static void pair_view_draw(Canvas* canvas, void* model) {
    const BtcpcPairViewModel* pair_model = model;
    canvas_clear(canvas);
    canvas_set_color(canvas, ColorBlack);
    ...
    canvas_draw_str(
        ...
        4,
        62,
        pair_model && pair_model->token[0] ? pair_model->token : "BTCPC:PAIR:00000000");
}

static bool navigation_callback(void* context) {
    BtcpcSensorApp* app = context;
    if(!app) return true;
    if(app->active_view == BTCPC_VIEW_PAIR || app->active_view == BTCPC_VIEW_DIAGNOSTICS) {
        app->active_view = BTCPC_VIEW_MENU;
        view_dispatcher_switch_to_view(app->view_dispatcher, BTCPC_VIEW_MENU);
        return true;
    }
    if(app->active_view == BTCPC_VIEW_RESULT) {
        if(app->cycle_active) {
...
static void show_result(BtcpcSensorApp* app, const char* title, const char* body) {
    if(!app || !app->text_box) return;
    snprintf(
        app->result,
        sizeof(app->result),
        "%s\n\n%s\n\nBack = return to menu",
        title ? title : "BTCPC Sensor",
        body ? body : "");
    text_box_set_text(app->text_box, app->result);
    text_box_set_focus(app->text_box, TextBoxFocusStart);
    app->active_view = BTCPC_VIEW_RESULT;
    view_dispatcher_switch_to_view(app->view_dispatcher, BTCPC_VIEW_RESULT);
}

static void probe_cycle_next(BtcpcSensorApp* app);
static bool broadcast_beacon(BtcpcSensorApp* app, const char* source, const char* reading);
static void send_diagnostics(BtcpcSensorApp* app);

static void show_pair_token(BtcpcSensorApp* app, const char* heading, const char* body) {
    if(!app) return;
    (void)body;
    update_pair_view_model(app, heading ? heading : "Pair Phone");
    app->active_view = BTCPC_VIEW_PAIR;
    view_dispatcher_switch_to_view(app->view_dispatcher, BTCPC_VIEW_PAIR);
}

static void show_diagnostics(BtcpcSensorApp* app) {
    if(!app || !app->diagnostics_submenu) return;
    app->active_view = BTCPC_VIEW_DIAGNOSTICS;
    view_dispatcher_switch_to_view(app->view_dispatcher, BTCPC_VIEW_DIAGNOSTICS);
}
...
static bool build_signed_packet(
    ...
    int written = snprintf(
        base,
        sizeof(base),
        "BTCPC1|dev=%s|sid=%08lX|seq=%lu|src=%s|%s",
        app->device_id,
...
static void send_diagnostics(BtcpcSensorApp* app) {
    ...
    furi_hal_subghz_set_frequency_and_path(BTCPC_SUBGHZ_FREQ);
    furi_hal_subghz_rx();
    furi_delay_ms(50);
...
static bool broadcast_beacon(BtcpcSensorApp* app, const char* source, const char* reading) {
    ...
    const SubGhzDevice* device = subghz_devices_get_by_name(BTCPC_SUBGHZ_DEVICE);
    ...
    subghz_devices_load_preset(
        ...
        preset_data);
    subghz_devices_set_frequency(device, BTCPC_SUBGHZ_FREQ);
...
static void probe_battery(BtcpcSensorApp* app) {
...
static void probe_cpu_temp(BtcpcSensorApp* app) {
    FuriHalAdcHandle* adc = furi_hal_adc_acquire();
    if(!adc) {
...
static void probe_gpio(BtcpcSensorApp* app) {
...
static void probe_nfc(BtcpcSensorApp* app) {
    if(furi_hal_nfc_acquire() != FuriHalNfcErrorNone) {
...
static void probe_ble(BtcpcSensorApp* app) {
    if(furi_hal_bt_is_active()) {
...
static void probe_subghz(BtcpcSensorApp* app) {
    furi_hal_subghz_reset();
    furi_hal_subghz_idle();
    furi_hal_subghz_set_path(FuriHalSubGhzPath433);
...
static void probe_cycle_next(BtcpcSensorApp* app) {
    if(!app) return;
    size_t count = sizeof(btcpc_cycle_order) / sizeof(btcpc_cycle_order[0]);
    if(count == 0) return;
    if(app->cycle_index >= count) app->cycle_index = 0;
    uint32_t probe_id = btcpc_cycle_order[app->cycle_index];
    app->cycle_index = (app->cycle_index + 1) % count;
    switch(probe_id) {
    case BTCPC_MENU_BATTERY:
        probe_battery(app);
        break;
    case BTCPC_MENU_CPU_TEMP:
        probe_cpu_temp(app);
        break;
    case BTCPC_MENU_GPIO:
        probe_gpio(app);
        break;
    case BTCPC_MENU_NFC:
        probe_nfc(app);
...
static BtcpcSensorApp* app_alloc(void) {
    BtcpcSensorApp* app = malloc(sizeof(BtcpcSensorApp));
    if(!app) return NULL;
    ...
    submenu_set_header(app->submenu, "BTCPC Sensor");
    submenu_add_item(app->submenu, "Pair Phone", BTCPC_MENU_PAIR, submenu_callback, app);
    submenu_add_item(app->submenu, "Unpair Phone", BTCPC_MENU_UNPAIR, submenu_callback, app);
    submenu_add_item(app->submenu, "Start Sensor Cycle", BTCPC_MENU_CYCLE, submenu_callback, app);
    submenu_add_item(app->submenu, "Diagnostics", BTCPC_MENU_DIAGNOSTICS, submenu_callback, app);

    submenu_set_header(app->diagnostics_submenu, "Diagnostics");
    submenu_add_item(app->diagnostics_submenu, "Save/Send Diagnostics", BTCPC_MENU_DIAGNOSTICS_SEND, submenu_callback, app);
    submenu_add_item(app->diagnostics_submenu, "Probe Battery", BTCPC_MENU_BATTERY, submenu_callback, app);
    submenu_add_item(app->diagnostics_submenu, "Probe CPU Temp", BTCPC_MENU_CPU_TEMP, submenu_callback, app);
    submenu_add_item(app->diagnostics_submenu, "Probe GPIO", BTCPC_MENU_GPIO, submenu_callback, app);
    submenu_add_item(app->diagnostics_submenu, "Probe NFC", BTCPC_MENU_NFC, submenu_callback, app);
    submenu_add_item(app->diagnostics_submenu, "Probe BLE", BTCPC_MENU_BLE, submenu_callback, app);
    submenu_add_item(app->diagnostics_submenu, "Probe SubGHz", BTCPC_MENU_SUBGHZ, submenu_callback, app);
    ...
    if(app->pair_view) {
        view_allocate_model(app->pair_view, ViewModelTypeLockFree, sizeof(BtcpcPairViewModel));
    ...
    view_dispatcher_add_view(app->view_dispatcher, BTCPC_VIEW_MENU, submenu_get_view(app->submenu));
    view_dispatcher_add_view(app->view_dispatcher, BTCPC_VIEW_RESULT, text_box_get_view(app->text_box));
    if(app->pair_view) {
        view_dispatcher_add_view(app->view_dispatcher, BTCPC_VIEW_PAIR, app->pair_view);
    }
    view_dispatcher_add_view(
        app->view_dispatcher,
        BTCPC_VIEW_DIAGNOSTICS,
...
static void app_free(BtcpcSensorApp* app) {
    if(!app) return;
    if(app->view_dispatcher) {
        view_dispatcher_set_tick_event_callback(app->view_dispatcher, NULL, 0);
        view_dispatcher_remove_view(app->view_dispatcher, BTCPC_VIEW_MENU);
        view_dispatcher_remove_view(app->view_dispatcher, BTCPC_VIEW_RESULT);
        if(app->pair_view) {
            view_dispatcher_remove_view(app->view_dispatcher, BTCPC_VIEW_PAIR);
        }
        view_dispatcher_remove_view(app->view_dispatcher, BTCPC_VIEW_DIAGNOSTICS);
...
int32_t btcpc_sensor_app(void* p) {
    (void)p;
    BtcpcSensorApp* app = app_alloc();
