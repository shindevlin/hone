 ...
 *   hone-mine --tools            (auto-starts on port HONE_MCP_PORT)
 *   hone tool mcp start          (start standalone)
 *
 * Protocol:
 ...
 * output to HONE-FS and return a trace_cid instead of raw output, so
 ...
 *   via HONE_MCP_CLI_TOOLS env var (JSON array of { name, command } objects).
 */
...
const PORT = parseInt(process.env.HONE_MCP_PORT) || 3101;
const NODE_ID = process.env.HONE_NODE_ID || process.env.HONE_MINER || "unknown";
...
const LOCAL_HANDLERS = {
  ...
  hone_fs_read: async ({ cid }) => {
    ...
    try {
      ...
      throw new Error("hone_fs_read: " + e.message);
    }
  },
  ...
  web_fetch: async ({ url, method, headers }) => {
    ...
    // Commit to HONE-FS and return CID (non-deterministic — must be committed)
...
function _parseCliTools() {
  try {
    return JSON.parse(process.env.HONE_MCP_CLI_TOOLS || "[]");
  } catch (_) { return []; }
}
...
function start() {
  ...
  _server.listen(PORT, "127.0.0.1", () => {
    console.log("[HONE MCP] Tool server listening on 127.0.0.1:" + PORT +
      " | node=" + NODE_ID +
      " | tools=" + _listTools().map(t => t.name).join(","));
  });

  _server.on("error", err => {
    console.error("[HONE MCP] Server error:", err.message);
  });
