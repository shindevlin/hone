/**
 * BTCPC P2P Relay — Cloudflare Durable Object
 * Shin Devlin
 *
 * WebSocket relay for BTCPC nodes behind NAT.
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      return new Response(JSON.stringify({ status: 'ok', service: 'btcpc-relay' }), {
    ...
    const id = env.RELAY.idFromName('btcpc-main-relay');
    const relay = env.RELAY.get(id);
    return relay.fetch(request);
  },
};

export class BTCPCRelay {
  ...
  async fetch(request) {
    ...
    if (url.pathname === '/v1/inference' && request.method === 'POST') {
      ...
      if (!auth || !auth.startsWith('Bearer btcpc_')) {
        return new Response(JSON.stringify({ error: 'Authorization required. Use Bearer btcpc_<key>' }), {
