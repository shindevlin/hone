 ...
 *
 * Universal bridge between Flipper Zero devices and the BTCPC chain.
 ...
 *
 *   BLE          — Flipper BTCPC app in BLE peripheral mode advertising
 ...
 *
 * Usage:
 *   BTCPC_JWT=<token> node scripts/btcpc-flipper-bridge.js
 *   BTCPC_JWT=<token> BTCPC_API=http://localhost:4242 node scripts/btcpc-flipper-bridge.js
 *   BTCPC_JWT=<token> RELAY_URL=http://other-node:7979/flipper node scripts/btcpc-flipper-bridge.js
 *
 * If RELAY_URL is set, all ingested readings are forwarded to another BTCPC
 * node instead of submitted directly (useful when this machine has no JWT).
 *
 * Environment:
 *   BTCPC_JWT          BTCPC auth token (required unless RELAY_URL is set)
 *   BTCPC_API          chain API base URL (default: https://btcpc.net)
 *   RELAY_URL          forward to another btcpc-flipper-bridge instead (optional)
 ...
 *   GATEWAY_NAME       gateway slug (default: btcpc-bridge)
...
const GATEWAY_NAME   = process.env.GATEWAY_NAME    || 'btcpc-bridge';
...
if (!JWT && !RELAY_URL) {
  console.error('[bridge] Set BTCPC_JWT for direct mode, or RELAY_URL=http://<node>:7979/flipper to relay.');
  process.exit(1);
}
...
async function ensureGateway() {
  ...
  try {
    const r = await apiPost('/api/gateways', {
      ...
      latitude: 0,
      longitude: 0,
      hardware_model: 'btcpc-flipper-bridge',
    });
...
function startBleTransport() {
  ...
  noble.on('discover', (peripheral) => {
    ...
    // Flipper BTCPC app advertises as "btcpc[-<wallet>]"
    if (!name || !name.toLowerCase().startsWith('btcpc')) return;

    // Wallet name is the suffix after "btcpc-" if present
    const wallet = name.toLowerCase().startsWith('btcpc-')
...
// The Flipper BTCPC app transmits a simple JSON payload via OOK/FSK.
...
function handleSdrMessage(decoded) {
  // rtl_433 output for a Flipper packet. The Flipper BTCPC app embeds a
  // JSON payload in its transmission; rtl_433 decodes the raw FSK frame.
  // Look for our custom fields: btcpc_wallet, rssi, freq.
  const wallet = decoded.btcpc_wallet || decoded.id || DEFAULT_WALLET;
...
async function main() {
  ...
  console.log(`[bridge] BTCPC Flipper Bridge  ${mode}`);
