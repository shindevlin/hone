#!/bin/bash
# ================================================================
# BTCPC Nebra SD Card Preparation Script
# Shin Devlin
#
# Downloads Raspberry Pi OS Lite, customizes it with BTCPC pre-
# installed, and writes the image to an SD card. After flashing,
# insert the card into the Nebra, plug in power + ethernet, and
# the device boots directly into a BTCPC LoRa gateway node.
#
# Usage:
...
say() { printf "${ORANGE}[btcpc-nebra]${RESET} %s\n" "$1"; }
...
# This script runs once on first boot, installs BTCPC, and sets up
# the LoRa gateway as a systemd service.
cat > "$ROOT_MNT/home/pi/btcpc-first-boot.sh" << 'FIRSTBOOT'
#!/bin/bash
# BTCPC Nebra First Boot Script
# Runs once, installs everything, starts the gateway
set -euo pipefail

LOG="/var/log/btcpc-first-boot.log"
exec > >(tee -a "$LOG") 2>&1

echo "[btcpc] First boot — $(date)"

# Wait for network
echo "[btcpc] Waiting for network..."
...
# Clone BTCPC
echo "[btcpc] Cloning BTCPC..."
if [ -d "/home/pi/.btcpc" ]; then
  cd /home/pi/.btcpc && git pull
else
  git clone https://github.com/shindevlin/btcpc.git /home/pi/.btcpc || true
fi
cd /home/pi/.btcpc
npm install --omit=dev

# Create .env
if [ ! -f "/home/pi/.btcpc/.env" ]; then
  cat > /home/pi/.btcpc/.env << ENV
NODE_ENV=production
PORT=3000
BTCPC_MINER=${BTCPC_NEBRA_ACCOUNT:-shindevlin}
BTCPC_ROLES=clock,storage,sensor,gateway
...
BTCPC_STORAGE_CAPACITY_GB=4
P2P_PORT=6942
ENV
fi

# Install systemd service
sudo cat > /etc/systemd/system/btcpc-nebra.service << UNIT
[Unit]
Description=BTCPC Nebra Gateway
...
ExecStart=/home/pi/.nvm/versions/node/$(node -v)/bin/node bin/btcpc-all
Restart=always
RestartSec=10
...
sudo systemctl enable btcpc-nebra
sudo systemctl start btcpc-nebra

echo "[btcpc] First boot complete — BTCPC gateway running"

# Remove this script from rc.local so it doesn't run again
sudo sed -i '/btcpc-first-boot/d' /etc/rc.local

FIRSTBOOT

chmod +x "$ROOT_MNT/home/pi/btcpc-first-boot.sh"

# Add to rc.local for first-boot execution
if [ -f "$ROOT_MNT/etc/rc.local" ]; then
  sed -i '/^exit 0/i bash /home/pi/btcpc-first-boot.sh &' "$ROOT_MNT/etc/rc.local"
else
  echo '#!/bin/bash' > "$ROOT_MNT/etc/rc.local"
  echo 'bash /home/pi/btcpc-first-boot.sh &' >> "$ROOT_MNT/etc/rc.local"
...
echo ""
echo "============================================================"
echo "  BTCPC Nebra SD Card Ready!"
echo "============================================================"
echo ""
...
echo "     (Node.js install + BTCPC clone + npm install)"
echo "  5. SSH in: ssh pi@<nebra-ip>"
echo "     Password: $NEBRA_PASS"
echo "  6. Check status: sudo systemctl status btcpc-nebra"
echo ""
