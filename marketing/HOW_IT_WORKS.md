...
4. **Miners get paid** in HONE tokens for providing correct answers
...
The user pays a small fee in HONE tokens (fractions of a cent per request).
...
Each 5-minute "epoch" (like a Bitcoin block), the network distributes newly minted HONE tokens to miners based on how much useful work they did. More AI work = more tokens. This is the mining reward.

### Step 5: The Chain Records Everything

Every inference request, every answer, every payment, and every mining reward is recorded on the HONE blockchain permanently. Anyone can verify any transaction by reading the chain.

## Key Concepts (Plain English)

### Epochs (= Bitcoin Blocks)
Every 5 minutes, a new "epoch" starts. This is HONE's version of a Bitcoin block. All the AI work done during that 5 minutes gets bundled, verified, and recorded.

### Mining Rewards
New HONE tokens are created every epoch and distributed to miners. The initial reward is ~243 HONE per epoch. This decreases over time (like Bitcoin's halving) until all 42 million tokens are mined.
...
### Inference
The AI process of generating an output from an input. When you ask ChatGPT a question, that's inference. HONE miners do the same thing, but decentralized.
...
## What You Can Do With HONE Tokens
...
- **Transfer to others** — send tokens to any HONE address
- **Use across chains** — HONE exists on Ethereum, Solana, Bitcoin, TON, and Hive simultaneously
