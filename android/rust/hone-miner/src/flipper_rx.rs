...
//!
//! Ported from `rust/hone-android/src/flipper_rx.rs` (the in-process P2P
//! micronode's Flipper receiver). hone-miner has no `Chain`/`net` — it talks
...
//!
//! Wire format (must match clients/hone-flipper/protocol/hone_protocol.h,
...
// Message types (from hone_protocol.h). Only Flipper->phone types handled.
...
fn frame_to_reading(frame: &Frame, owner: &str, flipper_pk_hex: &str) -> Option<SensorReading> {
    ...
    let (sensor_type, primary_value, values_json, unit, kind_tag) = match frame.msg_type {
        MSG_SUBGHZ_OBS => {
            // HoneSubGhzObs: freq_hz(u32 LE), rssi_dbm(i8), modulation(u8), bandwidth(u8)
...
/// (`rust/hone-node/src/tx.rs`, ~line 2496) exactly: same fields, same order,
...
/// (`post_sensor_commit` in `rust/hone-node/src/api.rs`). It builds a
...
mod tests {
    ...
    fn canonical_signing_message_matches_expected_shape() {
        ...
        // `canonical_signing_message` (rust/hone-node/src/tx.rs ~line 2496).
        let msg = build_canonical_signing_message(
            "hone-satoshi", "alice/flipper-abc-subghz", "alice",
            &"ab".repeat(32), 1, "continuous", "alice",
        );
        let parsed: serde_json::Value = serde_json::from_str(&msg).unwrap();
        assert_eq!(parsed["chain_id"], "hone-satoshi");
