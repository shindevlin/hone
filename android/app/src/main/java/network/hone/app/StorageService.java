...
/**
 * StorageService — hosts HONE-FS blobs from the phone.
 *
 * On start:
 *  1. Fetches the phone's assigned blob list from /api/storage/phone/assignments
 *  2. Downloads any blobs not already cached in getFilesDir()/hone-fs/
...
public class StorageService extends Service {
    ...
    private static final String CHANNEL_ID   = "hone_storage";
    ...
    public void onCreate() {
        ...
        blobDir = new File(getFilesDir(), "hone-fs");
        blobDir.mkdirs();
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        try {
            ...
            android.util.Log.w("HONEStorage", "startForeground failed: " + e.getMessage());
            stopSelf();
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel ch = new NotificationChannel(
                    CHANNEL_ID, "HONE Storage", NotificationManager.IMPORTANCE_LOW);
