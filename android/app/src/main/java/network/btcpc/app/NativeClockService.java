...
/**
 * NativeClockService — libp2p-based clock consensus via the Rust btcpc_miner library.
 *
 * Replaces the old WebSocket-relay approach with direct P2P epoch seals
 * submitted to the connected btcpc node API.
 */
public class NativeClockService extends Service {
    ...
    static {
        try {
            ...
            android.util.Log.w("BTCPCClock", "Rust library not found: " + e.getMessage());
        }
    }
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel ch = new NotificationChannel(
                    CHANNEL_ID, "BTCPC Clock Peer", NotificationManager.IMPORTANCE_LOW);
