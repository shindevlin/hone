        ...
        impl #name {
            ...
            fn __load() -> Self {
                ::btcpc_contract_sdk::env::storage_read(Self::__STATE_KEY)
                    ...
                    .unwrap_or_else(|| ::btcpc_contract_sdk::env::panic_str(
                        "Contract state not initialized — call init first"
                    ))
            }

            fn __save(&self) {
                let mut buf = ::btcpc_contract_sdk::__private::vec![];
...
pub fn btcpc_impl(_attr: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        #[allow(unused)]
        fn __to_json_bytes<T: ::btcpc_contract_sdk::Serialize>(v: &T) -> Option<::btcpc_contract_sdk::__private::Vec<u8>> {
        ...
        pub extern "C" fn __btcpc_dispatch() {
            let method = ::btcpc_contract_sdk::env::read_method_name();
            match method.as_str() {
                #(#init_arms)*
                #(#call_arms)*
                #(#view_arms)*
                other => ::btcpc_contract_sdk::env::panic_str(
                    &::btcpc_contract_sdk::__private::format!("method not found: {}", other)
                ),
            }
        }
    };
...
fn build_dispatch(m: &ImplItemFn) -> (TokenStream2, TokenStream2) {
    ...
    for (ident, ty) in &params {
        ...
        per_param.push(quote! {
            let #ident: #ty = ::btcpc_contract_sdk::serde_json::from_value(
