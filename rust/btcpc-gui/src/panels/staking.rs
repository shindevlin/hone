...
pub fn show(ui: &mut egui::Ui, data: &mut AppData) {
        ...
        ui.columns(3, |cols| {
            cols[0].vertical_centered(|ui| {
                ui.label(egui::RichText::new(
                    data.balance.map(|b| format!("{} BTCPC", dreams_to_btcpc(b)))
            ...
            cols[1].vertical_centered(|ui| {
                ui.label(egui::RichText::new(
                    data.staked.map(|s| format!("{} BTCPC", dreams_to_btcpc(s)))
                        .unwrap_or_else(|| "—".to_string())
            ...
            cols[2].vertical_centered(|ui| {
                let role_backed: u64 = data.role_positions.iter()
                    ...
                    .sum();
                ui.label(egui::RichText::new(
                    format!("{} BTCPC", dreams_to_btcpc(role_backed))
                ...
                egui::Grid::new("req_grid")
                    ...
                    .show(ui, |ui| {
                        for (role, min_val) in map {
                            ...
                            ui.label(egui::RichText::new(
                                format!("{} BTCPC min", dreams_to_btcpc(min_dreams))
    ...
    if !can_spend {
        ...
        if !data.role_positions.is_empty() {
                ...
                card(ui, |ui| {
                    ui.horizontal(|ui| {
                        ui.vertical(|ui| {
                            ...
                            ui.label(egui::RichText::new(
                                format!("{} — {} BTCPC", role_name, dreams_to_btcpc(amount))
                            ).small().color(DIM_TEXT));
                        });
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if is_selected {
                                ...
                                ui.add(egui::TextEdit::singleline(&mut data.forms.role_unstake_amount)
                                    .hint_text("0.0")
                                    .desired_width(80.0));
                                ui.label(egui::RichText::new("BTCPC").color(ORANGE).size(11.0));
                            } else {
                                if ui.add(
            ...
            if let Some((node, role, dreams)) = unstake_action {
                let base     = data.node_url.clone();
                let key_file = data.key_file.clone();
                data.forms.role_unstake_result = Some(match key_file {
                    None     => (false, "no key file".into()),
                    Some(kf) => match crate::sign::submit_role_stake(&base, &kf, &account, &node, &role, dreams, false) {
