...
pub fn cmd_balance(account: &str, token: Option<&str>) -> Result<()> {
    ...
    let token_name = token
        .map(|s| s.to_string())
        .or(resp.token)
        .unwrap_or_else(|| "BTCPC".to_string());

    if let Some(dreams) = resp.dreams {
        println!(
            "{} {}",
            "Balance:".bold(),
            dreams_to_btcpc(dreams).green()
        );
        ...
        println!("{} 0.0000000000 BTCPC", "Balance:".bold());
    }
    Ok(())
}
...
pub fn cmd_stake(account: &str) -> Result<()> {
    ...
    if let Some(dreams) = resp.dreams {
        println!("{} {}", "Staked:".bold(), dreams_to_btcpc(dreams).yellow());
