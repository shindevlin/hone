...
/// BTCPC_API_KEY is set to the account name (rotate-able tokens in a future release).
pub fn cmd_wallet_env(wallet_file: Option<&Path>, output: Option<&Path>) -> Result<()> {
    ...
    println!("  # set Authorization: Bearer {} in API calls", wf.account);
