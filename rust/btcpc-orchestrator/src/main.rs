use btcpc_api_catalog::{CatalogQuery, CatalogSnapshot, LinkStatus, PublicApiRecord};
use btcpc_orchestrator::{
...
struct ApiToolReport {
    ...
    attempt: btcpc_orchestrator::RuntimeAttempt,
    span: RuntimeSpan,
    attestation: Option<btcpc_orchestrator::RuntimeAttestation>,
    ...
    match probe {
        Ok(probe) => {
            ...
            let attestation = if (200..400).contains(&probe.status_code) {
                ...
                store.fail_attempt(
                    &attempt.attempt_id,
                    btcpc_orchestrator::AttemptStatus::Failed,
                    now + 4,
                )?;
                None
            };
