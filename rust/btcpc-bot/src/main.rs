...
async fn main() -> Result<()> {
    ...
    let bot_token = std::env::var("BTCPC_BOT_TOKEN")
        .expect("BTCPC_BOT_TOKEN environment variable must be set");

    let api_url = std::env::var("BTCPC_API_URL")
        .unwrap_or_else(|_| "http://localhost:4242".to_string());

    let default_account = std::env::var("BTCPC_ACCOUNT").ok();

    info!("Starting BTCPC bot");
