...
///
/// When the chain's genesis timestamp predates its actual go-live (e.g. a
/// commemorative genesis date with launch weeks later), the epochs between
/// genesis and the first quorum-sealed epoch were never sealed, so their block
/// rewards were never paid. Because the supply cap is accounted by epoch number
/// (see `block_reward_at`), those rewards would otherwise be forfeited. This is
/// the exact amount that must be credited ONCE to `RECYCLE_FUND_ACCOUNT` — the
/// perpetual end-of-chain reward pool — so nothing is lost and realized supply
/// still reaches `SUPPLY_CAP_HUNITS`.
///
/// Era- and cap-aware (delegates to `block_reward_at`), so it stays correct even
/// if the gap ever spanned an era boundary or brushed the supply cap. Intended
/// to be called exactly once, at cold start, with the consensus-agreed first
/// sealed epoch — never per-tick.
...
mod tests {
    ...
    fn genesis_gap_recycle_basic() {
        // No gap (first sealed epoch is 1) → nothing to recycle.
        assert_eq!(genesis_gap_recycle_hunits(0), 0);
        assert_eq!(genesis_gap_recycle_hunits(1), 0);
        // First sealed epoch 2 → only epoch 1's reward.
        assert_eq!(genesis_gap_recycle_hunits(2), BLOCK_REWARD_HUNITS);
        // A realistic backdated launch: genesis 2026-07-04, live ~23 days later.
        // first sealed epoch 66_571 → epochs 1..66_571 = 66_570 full rewards.
        assert_eq!(
            genesis_gap_recycle_hunits(66_571),
            66_570 * BLOCK_REWARD_HUNITS
        );
        // 66_570 × 2 HONE = 133_140 HONE (1 HONE = 10^10 hunits).
        assert_eq!(
            genesis_gap_recycle_hunits(66_571),
            133_140 * 10_000_000_000
        );
    }

    #[test]
    fn genesis_gap_recycle_matches_block_reward_sum() {
        // Must equal the direct per-epoch sum for any first-sealed epoch.
    ...
    fn genesis_gap_plus_realized_preserves_cap() {
        // Backdating forfeits nothing beyond the pre-existing genesis-marker
        // quirk: the gap (credited to recycle) plus the rewards payable from the
        // first sealed epoch onward equal the entire new-supply total from
        // epoch 1. Epoch 0 is the genesis marker — block_reward_at(0) is counted
        // in the cap sum but never actually paid, with or without backdating.
        let f = 66_571u64;
