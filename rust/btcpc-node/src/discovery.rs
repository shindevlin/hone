...
use btcpc_types::TESTNET_CHAIN_ID;
...
// ── Hive self-announce ────────────────────────────────────────────────────────
//
// If BTCPC_HIVE_POSTING_KEY (WIF) is set, the node posts a custom_json to the
...
// For operator-signed publishing, set BTCPC_HIVE_POSTING_KEY.
//
...
pub async fn announce_to_hive(chain_id: &str, node_id: &str) {
    ...
    let posting_key_wif = match std::env::var("BTCPC_HIVE_POSTING_KEY") {
        Ok(v) if !v.is_empty() => v,
        _ => {
            info!("discovery: BTCPC_HIVE_POSTING_KEY not set — skipping Hive announcement");
            return;
        }
    };
