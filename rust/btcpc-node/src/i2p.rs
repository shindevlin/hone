...
//!
//! Env vars:
//!   BTCPC_I2P              — "true" to activate
//!   BTCPC_I2P_SAM_HOST     — SAM bridge host (default: 127.0.0.1)
//!   BTCPC_I2P_SAM_PORT     — SAM bridge TCP port (default: 7656)
...
/// Initialise the I2P datagram transport.  Returns `None` if `BTCPC_I2P` is
...
pub async fn start_i2p(
    ...
    if std::env::var("BTCPC_I2P").as_deref() != Ok("true") {
        return None;
    }

    let sam_host = std::env::var("BTCPC_I2P_SAM_HOST")
        .unwrap_or_else(|_| "127.0.0.1".to_owned());
    let sam_port: u16 = std::env::var("BTCPC_I2P_SAM_PORT")
