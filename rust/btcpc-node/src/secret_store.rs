...
const MAGIC: &[u8] = b"BTCPC_SECRETS_V1";
...
impl SecretStore {
    /// Open or create the secret store. Key is derived from `BTCPC_SECRETS_PASSPHRASE`
    /// env var if set, otherwise from `hw_fingerprint`.
    pub fn open(data_dir: &Path, hw_fingerprint: &str, store: Store) -> Result<Self> {
        let key_material = std::env::var("BTCPC_SECRETS_PASSPHRASE")
