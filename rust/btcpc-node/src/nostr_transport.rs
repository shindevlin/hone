...
//!
//! Env vars:
//!   BTCPC_NOSTR              — "true" to activate
//!   BTCPC_NOSTR_RELAYS       — comma-separated WSS URLs
//!                              (default: wss://relay.damus.io,wss://nos.lol,wss://relay.nostr.band)
//!   BTCPC_NOSTR_KIND         — event kind integer (default: 21337)
...
/// Default Nostr event kind for BTCPC ledger entries.
...
/// Initialise the Nostr relay transport.  Returns `None` if `BTCPC_NOSTR` is
...
pub async fn start_nostr(
    ...
    if std::env::var("BTCPC_NOSTR").as_deref() != Ok("true") {
        return None;
    }
...
async fn try_start(
    ...
    let kind_num: u16 = std::env::var("BTCPC_NOSTR_KIND")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(DEFAULT_KIND);

    let relay_urls: Vec<String> = std::env::var("BTCPC_NOSTR_RELAYS")
