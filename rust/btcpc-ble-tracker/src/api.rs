...
impl NodeClient {
    ...
    /// Store an encrypted blob in BTCPC-FS, returning its CID.
    /// Placeholder: POSTs to /api/fs/store; returns a synthetic CID until BTCPC-FS is wired.
    pub async fn store_blob(&self, _owner: &str, data: &str) -> anyhow::Result<String> {
        use sha2::{Digest, Sha256};
        // Stub: derive a deterministic CID from the content hash until BTCPC-FS is live.
