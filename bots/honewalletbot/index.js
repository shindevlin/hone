...
bot.onText(/\/link\s+(\S+)/, async (msg, match) => {
  try {
    ...
    bot.sendMessage(msg.chat.id, [
      ...
      ``,
      `Sign: \`${data.challenge}\``,
      `CLI: \`node bin/hone-cli sign-challenge ${data.challenge}\``,
...
bot.onText(/\/balance/, async (msg) => {
  try {
    ...
    bot.sendMessage(msg.chat.id, [
      ...
      `Balance: \`${fmt(data.balance, 10)} HONE\``,
...
bot.onText(/\/history/, async (msg) => {
  try {
    ...
    const lines = data.transactions.map(tx => `  ${tx.direction === 'in' ? '\u{2B06}' : '\u{2B07}'} ${tx.type}: ${fmt(tx.amount)} HONE`);
...
bot.onText(/\/claim/, async (msg) => {
  try {
    ...
    bot.sendMessage(msg.chat.id, `Claimed 1 HONE. Balance: \`${fmt(data.balance, 10)}\``, { parse_mode: 'Markdown' });
  } catch (err) {
    bot.sendMessage(msg.chat.id, `Error: ${err.response?.data?.error || err.message}`);
  }
});
...
bot.onText(/\/projects/, async (msg) => {
  try {
    ...
    const lines = data.projects.map(p => `  ${p.verified ? '\u{2705}' : '\u{23F3}'} *${p.name}* — ${fmt(p.balance, 10)} HONE | ${p.totalRequests} req`);
...
bot.onText(/\/mining/, async (msg) => {
  try {
    ...
    const lines = data.proofs.slice(0, 5).map(p => `  Block ${p.block}: +${fmt(p.reward)} HONE | ${p.tokens} tokens`);
...
bot.onText(/\/reward/, async (msg) => {
  try {
    ...
    bot.sendMessage(msg.chat.id, `Reward: \`${fmt(data.reward, 10)} HONE\` (epoch ${data.epoch})`, { parse_mode: 'Markdown' });
  } catch (err) {
    bot.sendMessage(msg.chat.id, `Error: ${err.response?.data?.error || err.message}`);
  }
});
...
app.get('/health', (_req, res) => res.json({ status: 'ok', service: 'honewalletbot' }));
...
// ── Start ──
app.listen(PORT, () => console.log(`[honewalletbot] Alert webhook on port ${PORT}`));
console.log('[honewalletbot] @honewalletbot is live (API mode)');
