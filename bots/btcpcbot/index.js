...
if (!BOT_TOKEN) { console.error('BTCPC_BOT_TOKEN required'); process.exit(1); }
...
async function startLoginUserPass(chatId) {
  ...
  await bot.sendMessage(chatId, 'Enter your BTCPC username:');
}
...
async function showMainMenu(chatId, username, balance) {
  ...
  const balStr = balance !== undefined ? `\nBalance: *${fmt(balance)} BTCPC*` : '';
  await bot.sendMessage(chatId,
    `🟠 *BTCPC — ${username}*${balStr}`,
    { parse_mode: 'Markdown', ...kb([
      [
...
async function showAddressBook(chatId, wallets) {
  const lines = [
    ...
    `🔶 BTCPC:    \`${wallets.btcpc || wallets.username || '—'}\``,
...
async function showEarnOptions(chatId) {
  await bot.sendMessage(chatId,
    'How would you like to earn BTCPC?',
    kb([
...
bot.on('callback_query', async (query) => {
  ...
  if (action === 'ACT_BALANCE') {
    ...
    await bot.sendMessage(chatId, [
      ...
      ``,
      `Spendable: \`${fmt(data.balance)} BTCPC\``,
      `Staked:    \`${fmt(data.staked)} BTCPC\``,
      `Total:     \`${fmt(data.balance + data.staked)} BTCPC\``,
  ...
  if (action === 'ACT_RECEIVE') {
    ...
    if (!data) return;
    await bot.sendMessage(chatId,
      `📥 *Your BTCPC address:*\n\`${data.username}\`\n\nShare your username to receive BTCPC.`,
      { parse_mode: 'Markdown', ...kb([[{ text: '← Menu', callback_data: 'MENU' }]]) }
    );
    return;
  }
  ...
  if (action === 'ACT_BRIDGE') {
    await bot.sendMessage(chatId,
      '💱 *Bridge*\n\nCross-chain bridging coming soon.\nVisit btcpc.net for bridge partners.',
      { parse_mode: 'Markdown', ...kb([[{ text: '← Menu', callback_data: 'MENU' }]]) }
    );
    return;
  }
  ...
  const EARN_INFO = {
    EARN_CLOCK: {
      title: '⏰ Clock Node',
      body: 'Run a clock node on any device — laptop, Raspberry Pi, phone.\n\n`npm install -g btcpc-clock && btcpc-clock --account <username>`\n\nMore: btcpc.net/install/clock'
    },
    EARN_MINE: {
      title: '🖥️ GPU Mining',
      body: 'Mine BTCPC with any Ollama-compatible GPU.\n\n`npm install -g btcpc && btcpc-mine --miner <username>`\n\nRequires: Ollama + any model (qwen, llama, mistral, gemma, deepseek...)\nMore: btcpc.net/install/mine'
    },
    EARN_STORE: {
      title: '💾 Storage Node',
      body: 'Earn BTCPC by hosting files for the BTCPC-FS layer.\n\n`npm install -g btcpc-storage && btcpc-storage --account <username>`\n\nMore: btcpc.net/install/storage'
    },
    EARN_IOT: {
      title: '📡 IoT Sensor / Gateway',
      body: 'Connect sensors or run a gateway node to earn BTCPC for providing real-world data.\n\nMore: btcpc.net/install/iot'
    }
  };
  ...
  if (action === 'SET_ADDRESSES') {
    ...
    const wallets = {
      ...
      btcpc: data.addresses?.btcpc || data.username || '—',
  ...
  if (action === 'EXPORT_DO') {
    await bot.sendMessage(chatId,
      '⚠️ Mnemonic export is not available via Telegram yet. Use the CLI (`btcpc-cli export`) to export your backup phrase.',
  ...
  if (action === 'SET_LINK') {
    ...
    await bot.sendMessage(chatId,
      '🔗 *Link Account*\n\nEnter the BTCPC username to link to this Telegram account:',
  ...
  if (action === 'EARN_BTCPC') return showEarnOptions(chatId);
...
bot.on('message', async (msg) => {
  ...
  if (s.step === 'SEND_AMOUNT') {
    ...
    await bot.sendMessage(chatId,
      '⚠️ Sending BTCPC via Telegram is not available yet. Use the CLI or web wallet to send transactions.',
      { parse_mode: 'Markdown', ...kb([[{ text: '← Menu', callback_data: 'MENU' }]]) }
    );
    setState(chatId, 'MAIN_MENU');
    return;
...
// ── Error handling ────────────────────────────────────────────────────────────
bot.on('polling_error', (err) => console.error('[btcpcbot] Polling error:', err.code || err.message));
process.on('uncaughtException',  (err)    => console.error('[btcpcbot] Uncaught:', err.message));
process.on('unhandledRejection', (reason) => console.error('[btcpcbot] Unhandled:', reason));

console.log('[btcpcbot] @btcpcbot is live — conversational inline-keyboard UX');
